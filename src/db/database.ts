  ...
  public upsertSupplierProfile(profile: Partial<DBProfileSupplier> & { user_id: string; company_name: string }): DBProfileSupplier {
    ...
    if (existingIndex >= 0) {
      ...
      const created: DBProfileSupplier = {
        ...
        gst_number: profile.gst_number || '',
        year_established: profile.year_established || String(new Date().getFullYear()),
        employee_count: profile.employee_count || 'Contact for Details',
        service_areas: profile.service_areas || ['Pan India'],
        categories: profile.categories || [],
        response_rate: profile.response_rate ?? 95,
        avg_response_time: profile.avg_response_time ?? 2.0,
        profile_completion_pct: profile.profile_completion_pct ?? 85,
        ...
        is_gst_verified: profile.is_gst_verified ?? false,
        is_iso_certified: profile.is_iso_certified ?? false,
        status: profile.status || 'pending_verification',
        is_verified_supplier: profile.is_verified_supplier ?? true,
        category: profile.category || (profile.categories && profile.categories[0]) || '',
        subcategory: profile.subcategory || '',
        brand_name: profile.brand_name || profile.company_name,
        about: profile.about || '',
        onboarding_status: profile.onboarding_status || 'review',
        reviewed_at: null,
        approved_at: null,
        verification_notes: null,
        created_at: now,
