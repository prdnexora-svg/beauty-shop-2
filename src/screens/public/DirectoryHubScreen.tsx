...
const CATEGORIES = [
  ...
  { id: 'c2', name: 'Private Label Brands', icon: <Sparkles className="w-5 h-5" />, count: '450+ Brands', color: 'bg-[#F5EEF8] text-[#6B2D8C]' },
  { id: 'c3', name: 'Packaging & Containers', icon: <Package className="w-5 h-5" />, count: '85 Suppliers', color: 'bg-purple-50 text-purple-700' },
...
export const DirectoryHubScreen: React.FC<DirectoryHubScreenProps> = ({
  ...
  return (
    <div className="bg-[#FDFBF7] min-h-screen">
      ...
      {/* Search & Hero Section */}
      <div className="bg-white border-b border-[#E8DEEF] py-12 px-6 md:px-12">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#F5EEF8] text-[#6B2D8C] text-[10px] font-bold uppercase tracking-widest mb-4">
              <Building2 className="w-3.5 h-3.5" />
              Verified Beauty Business Directory
            </div>
            <h1 className="text-3xl md:text-5xl font-black text-[#2A0E3F] tracking-tight leading-[1.1]">
              Find the Right Manufacturing Partner for Your Beauty Brand
            </h1>
            <p className="text-sm md:text-base text-[#5B4A6E] mt-4 leading-relaxed">
          ...
          <div className="flex flex-col md:flex-row gap-3 max-w-4xl pt-4">
            <div className="flex-1 relative">
              ...
              <input 
                ...
                className="w-full bg-[#FDFBF7] border border-[#E8DEEF] focus:border-[#C9A961] focus:outline-none rounded-xl pl-12 pr-4 py-4 text-sm"
              />
            </div>
            <button 
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-12 space-y-20">
        ...
        <section className="space-y-8">
          <div className="flex justify-between items-end">
            <div>
              <h2 className="text-2xl font-black text-[#2A0E3F]">Browse by Sourcing Category</h2>
              <p className="text-xs text-[#5B4A6E] mt-1">Specialized manufacturing hubs for every stage of beauty product development.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {CATEGORIES.map((cat) => (
              <button
                ...
                className="bg-white border border-[#E8DEEF] hover:border-[#6B2D8C] hover:shadow-lg p-6 rounded-2xl transition-all text-left group cursor-pointer"
              >
                ...
                <h3 className="font-bold text-sm text-zinc-900 group-hover:text-[#6B2D8C] transition-colors">{cat.name}</h3>
                <div className="flex justify-between items-center mt-2">
                  <span className="text-[11px] text-[#5B4A6E] font-semibold">{cat.count}</span>
                  <ChevronRight className="w-4 h-4 text-zinc-300 group-hover:text-[#6B2D8C] transition-transform group-hover:translate-x-1" />
                </div>
              </button>
            ))}
          </div>
        ...
        <section className="space-y-8 bg-[#FDFBF7] p-8 md:p-12 rounded-[32px] border border-[#E8DEEF] relative overflow-hidden">
          <div className="absolute top-0 right-0 p-8 opacity-5">
            <TrendingUp className="w-64 h-64" />
          </div>
          
          <div className="relative z-10 space-y-1">
            <div className="flex items-center gap-2 text-[#6B2D8C] font-bold text-[10px] uppercase tracking-widest mb-1">
              <span className="w-2 h-2 rounded-full bg-[#6B2D8C] animate-pulse"></span>
              Live Sourcing Board
            </div>
            <h2 className="text-2xl font-black text-[#2A0E3F]">Recent Buy Requirements</h2>
            <p className="text-xs text-[#5B4A6E]">Connect with active buyers and brands looking for manufacturing services right now.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
            {RECENT_RFQS.map((rfq) => (
              <div key={rfq.id} className="bg-white p-5 rounded-2xl border border-[#E8DEEF] hover:border-[#6B2D8C] transition-all group">
                <div className="flex justify-between items-start mb-3">
                  <span className="text-[9px] font-bold text-[#6B2D8C] bg-[#F5EEF8] px-2 py-0.5 rounded-full uppercase tracking-wider">{rfq.type}</span>
                  <div className="flex items-center gap-1 text-[9px] text-[#7E6C96]">
                    <Clock className="w-3 h-3" />
                    {rfq.time}
                  </div>
                </div>
                <h4 className="font-extrabold text-sm text-zinc-950 leading-snug group-hover:text-[#6B2D8C] transition-colors mb-2">{rfq.product}</h4>
                <div className="space-y-1 text-[11px] text-[#5B4A6E]">
                ...
                <button 
                  ...
                  className="w-full mt-4 bg-[#FDFBF7] border border-[#E8DEEF] group-hover:bg-[#6B2D8C] group-hover:text-white group-hover:border-[#6B2D8C] py-2 rounded-xl text-[11px] font-bold transition-all"
                >
                  Apply to Quote
                </button>
              </div>
        ...
        <section className="space-y-8">
          <div className="flex justify-between items-end">
            <div>
              <h2 className="text-2xl font-black text-[#2A0E3F]">Top-Rated Partner Manufacturers</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {FEATURED_SUPPLIERS.map((supp) => (
              <div 
                ...
                className="bg-white border border-[#E8DEEF] rounded-2xl overflow-hidden hover:shadow-xl transition-all group cursor-pointer"
              >
                ...
                <div className="p-5 space-y-3">
                  <div>
                    <h3 className="font-extrabold text-sm text-zinc-950 group-hover:text-[#6B2D8C] transition-colors">{supp.name}</h3>
                    <div className="flex items-center gap-1.5 text-[11px] text-[#5B4A6E] mt-0.5">
                      <Building2 className="w-3.5 h-3.5" />
                      <span>{supp.type}</span>
                    </div>
                  </div>
                  
                  <div className="flex items-center gap-1 text-[11px] text-[#7E6C96]">
                    <MapPin className="w-3.5 h-3.5" />
                    <span>{supp.location}</span>
                  </div>

                  <div className="pt-3 border-t border-[#E8DEEF] flex flex-wrap gap-1.5">
                    {supp.certifications.map((cert) => (
                      <span key={cert} className="text-[9px] bg-[#F6F1FA] text-zinc-600 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">
                        {cert}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="pt-2">
                    <span className="text-[10px] text-[#6B2D8C] font-bold">Specialty: {supp.specialty}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <section className="bg-white border border-[#E8DEEF] rounded-[32px] overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="p-8 md:p-16 space-y-8">
              <div className="space-y-4">
                <h2 className="text-3xl font-black text-[#2A0E3F] tracking-tight">The Verified Sourcing Advantage</h2>
                <p className="text-sm text-[#5B4A6E] leading-relaxed">
                ...
                {[
                  ...
                  <div key={idx} className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-[#5B4A6E] mt-0.5">{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-[#F5EEF8] relative overflow-hidden flex items-center justify-center p-12">
               <div className="relative z-10 text-center space-y-6">
                 <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mx-auto shadow-xl">
                   <Award className="w-10 h-10 text-[#6B2D8C]" />
                 </div>
                 <div>
                    <h3 className="text-xl font-black text-[#6B2D8C]">Build with Trust</h3>
                    <p className="text-xs text-[#4A2560] mt-2 max-w-xs mx-auto">
                      Whether you are an established brand or a startup, our directory connects you to partners who grow with you.
                    </p>
                 </div>
                 <button 
                  onClick={() => onNavigate('onboarding')}
                  className="bg-[#6B2D8C] text-white font-black text-xs px-10 py-4 rounded-xl shadow-lg hover:bg-[#4A2560] transition-all"
                 >
                   Get Your Business Verified
                 </button>
               </div>
               {/* Decorative Circles */}
               <div className="absolute top-[-50px] right-[-50px] w-64 h-64 border-4 border-[#6B2D8C]/10 rounded-full"></div>
               <div className="absolute bottom-[-50px] left-[-50px] w-64 h-64 border-4 border-[#6B2D8C]/10 rounded-full"></div>
            </div>
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-12">
          <div className="bg-[#2A0E3F] text-white p-8 rounded-2xl space-y-6 relative overflow-hidden group">
          ...
          <div className="bg-[#6B2D8C] text-white p-8 rounded-2xl space-y-6 relative overflow-hidden group">
            <div className="absolute right-[-20px] bottom-[-20px] opacity-10 group-hover:scale-110 transition-transform duration-500">
            ...
            <div className="relative z-10 space-y-4">
              ...
              <button 
                ...
                className="bg-white text-[#6B2D8C] font-black text-xs px-6 py-3 rounded-lg hover:bg-neutral-50 transition-all cursor-pointer"
              >
                Post RFQ Now
              </button>
            </div>
